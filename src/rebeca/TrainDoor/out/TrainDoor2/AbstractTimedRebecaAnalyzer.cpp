#include "AbstractTimedRebecaAnalyzer.h"
			...
			#define _ref_controller (dynamic_cast<ControllerActor*>(rebecs[0]))
	    			#define _ref_train (dynamic_cast<TrainActor*>(rebecs[1]))
	    			#define _ref_door (dynamic_cast<DoorActor*>(rebecs[2]))
...
void AbstractTimedRebecaAnalyzer::instantiationPart() {
	...
	byte maxRebecID = 0;
        	ControllerActor* _ref_controller = new ControllerActor(maxRebecID++, "controller", 5, 0, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 15    											, hashmapSize
    											);
		        	    		_ref_controller->setPriority(1);
    		        rebecs.push_back(_ref_controller);
        referenceToRebecs->push_back(_ref_controller);
	    	TrainActor* _ref_train = new TrainActor(maxRebecID++, "train", 10, 0, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 25    											, hashmapSize
    											);
		        	    		_ref_train->setPriority(2);
    		        rebecs.push_back(_ref_train);
        referenceToRebecs->push_back(_ref_train);
	    	DoorActor* _ref_door = new DoorActor(maxRebecID++, "door", 10, 0, *referenceToRebecs 
    											#ifdef DEBUG_LEVEL_2 
    											, analysisOutputStream 
    											#endif
    											, 25    											, hashmapSize
    											);
		        	    		_ref_door->setPriority(2);
